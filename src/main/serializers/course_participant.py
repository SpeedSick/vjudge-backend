...
class CourseParticipantApproveSerializer(serializers.Serializer):
    ...
    def approve(self):
        course_participant = CourseParticipant.objects.filter(student=self.validated_data['student'],
                                                              course=self.validated_data['course']).last()
        if course_participant is None:
            course_participant, created = CourseParticipant.objects.get_or_create(
